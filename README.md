...
| literature rows inside the 90% Monte-Carlo interval | **16 of 44** (median width 0.99 dex); **16 of 43** out of sample; 1 row not evaluable — glucosone in the amine-free pot, whose only route carries no flux and has no band |
...
- **Absolute concentrations are unreliable.** On the union panel the kinetic core lands 10/45 rows within 3x (median fold error 9.31x, worst 9.63e+03x); out of sample -- every row a core fit read removed -- 9/44 (median 10.6x). Nothing in this repository licenses a ppb number as a specification. The core's 90% Monte-Carlo interval covers 16/44 evaluable literature rows (1 not evaluable: the no lane carries no sampled uncertainty), 16/43 out of sample.
