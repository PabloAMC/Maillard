...
| literature rows inside the 90% Monte-Carlo interval | **9 of 43** evaluable (median width 1.14 dex); **9 of 42** out of sample; 6 rows not evaluable |
...
- **Absolute concentrations are unreliable.** On the union panel the kinetic core lands 6/49 rows within 3x (median fold error 50.1x, worst 8.39e+10x); out of sample -- every row a core fit read removed -- 5/48 (median 51.2x). Nothing in this repository licenses a ppb number as a specification. The core's 90% Monte-Carlo interval covers 9/43 evaluable literature rows (6 not evaluable: the no lane carries no sampled uncertainty), 9/42 out of sample.
...
| Absolute concentration interval (90% CI) | every lane with sampled uncertainty | 9/43 evaluable literature rows inside; 9/42 out of sample; 6 not evaluable<br/><sub>results/validation/core_prediction_uncertainty.json (n=200); the no lane has no sampled uncertainty</sub> | **do-not-use** |
