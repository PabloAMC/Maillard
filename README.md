...
| | kinetic core |
| --- | --- |
| rows within 3x of the measurement | **10 of 45** (median fold error 9.3x, geometric mean 18x) |
| **out of sample** — every row a core fit read removed | **9 of 44** (median 10x); since the primary-evidence refit only one scored row is a fit row |
...
| literature rows inside the 90% Monte-Carlo interval | **12 of 42** (median width 0.92 dex); **12 of 41** out of sample; 3 rows not evaluable — the 3-deoxyglucosone limb has no prior rows, so the envelope publishes those three as exact |
...
- **Absolute concentrations are unreliable.** On the union panel the kinetic core lands 10/45 rows within 3x (median fold error 9.31x, worst 9.63e+03x); out of sample -- every row a core fit read removed -- 9/44 (median 10.6x). Nothing in this repository licenses a ppb number as a specification. The core's 90% Monte-Carlo interval covers 12/42 evaluable literature rows (3 not evaluable: the no lane carries no sampled uncertainty), 12/41 out of sample.
...
| Absolute concentration interval (90% CI) | every lane with sampled uncertainty | 12/42 evaluable literature rows inside; 12/41 out of sample; 3 not evaluable<br/><sub>results/validation/core_prediction_uncertainty.json (n=200); the no lane has no sampled uncertainty</sub> | **do-not-use** |
...
| Any claim of benchmark-grade agreement | the union panel: trust loop + hold-outs + matrix bundles | 0/37 strict-ready (none); 10/45 rows within 3x, out-of-sample 9/44<br/><sub>recomputed live; strict-ready is the repository's own passing bar</sub> | **do-not-use** |
