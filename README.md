...
The numerator did move, once, and it is the only accuracy gain here: **three rows entered the 3x
band together** when a beverage's own unheated column was declared as what the pot started with.
Between 36 % and 42 % of every level that paper prints was in the beverage before any heat, and the
model had been charged for the raw material. None of the three is a fit row. The median fold error
falls from 31x to 9.3x across all of this, and most of that fall is the refusals, not the fix.

| | kinetic core |
| --- | --- |
| rows within 3x of the measurement | **7 of 39** (median fold error 9.3x, geometric mean 20x) |
| **out of sample** — every row a core fit read removed | **6 of 38** (median 11x); since the primary-evidence refit only one scored row is a fit row |
| by lane, within 3x | acrylamide 2/12 · sulfur 2/19 · lipid 3/7 · trunk 0/1 |
| rows refused rather than answered | **25**, of which 7 are pots nobody heated (the refusal names its cure: declare what the pot started with) |
...
- **Absolute concentrations are unreliable.** On the union panel the kinetic core lands 7/39 rows within 3x (median fold error 9.31x, worst 9.63e+03x); out of sample -- every row a core fit read removed -- 6/38 (median 10.6x). Nothing in this repository licenses a ppb number as a specification. The core's 90% Monte-Carlo interval covers 10/42 evaluable literature rows (0 not evaluable: the no lane carries no sampled uncertainty), 10/41 out of sample.
...
| Absolute concentration (ppb) | protein matrix, lipid-derived aldehydes [lipid lane] | 3/7 rows within 3x, median 3.04x<br/><sub>recomputed live on the union panel; an absolute is never trust by rule</sub> | **do-not-use** |
...
| Absolute concentration interval (90% CI) | every lane with sampled uncertainty | 10/42 evaluable literature rows inside; 10/41 out of sample; 0 not evaluable<br/><sub>results/validation/core_prediction_uncertainty.json (n=200); the no lane has no sampled uncertainty</sub> | **do-not-use** |
...
| Any claim of benchmark-grade agreement | the union panel: trust loop + hold-outs + matrix bundles | 0/37 strict-ready (none); 7/39 rows within 3x, out-of-sample 6/38<br/><sub>recomputed live; strict-ready is the repository's own passing bar</sub> | **do-not-use** |
