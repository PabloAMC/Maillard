...
| literature rows inside the 90% Monte-Carlo interval | **11 of 39** (median width 0.92 dex); **11 of 38** out of sample; every row is now evaluable (5 were not, before the envelope gained its missing priors) |
...
- **Absolute concentrations are unreliable.** On the union panel the kinetic core lands 7/39 rows within 3x (median fold error 9.31x, worst 9.63e+03x); out of sample -- every row a core fit read removed -- 6/38 (median 10.6x). Nothing in this repository licenses a ppb number as a specification. The core's 90% Monte-Carlo interval covers 11/39 evaluable literature rows (0 not evaluable: the no lane carries no sampled uncertainty), 11/38 out of sample.
...
| Absolute concentration interval (90% CI) | every lane with sampled uncertainty | 11/39 evaluable literature rows inside; 11/38 out of sample; 0 not evaluable<br/><sub>results/validation/core_prediction_uncertainty.json (n=200); the no lane has no sampled uncertainty</sub> | **do-not-use** |
