...
| literature rows inside the 90% Monte-Carlo interval | **5 of 33** evaluable (median width 1.38 dex); **5 of 32** out of sample; 6 rows not evaluable |
| direction / ranking skill (74-claim literature panel) | **17 of 28** strictly independent evaluable claims; **13 of 23** with pH set aside, **4 of 5** on pH; water-activity comparisons are refused; 26 independent claims not evaluable |
...
**Directional and ranking claims are the product, and the core scores 17 of 28 on them.**
[`core_directional_scores.md`](results/validation/core_directional_scores.md) runs every claim of
the 74-claim literature panel ([`directional_claims_panel.yml`](docs/validation/directional_claims_panel.yml))
through the same front door a user calls. Nineteen claims are prose-only and 16 more are not
...
- **Absolute concentrations are unreliable.** On the union panel the kinetic core lands 4/39 rows within 3x (median fold error 29.5x, worst 3.34e+04x); out of sample -- every row a core fit read removed -- 3/38 (median 31.9x). Nothing in this repository licenses a ppb number as a specification. The core's 90% Monte-Carlo interval covers 5/33 evaluable literature rows (6 not evaluable: the no lane carries no sampled uncertainty), 5/32 out of sample.
- **Directional and ranking claims are the product, and on the kinetic core they score 17/28 on strictly independent literature claims** (29 independent claims not evaluable: refused arms, prose-only claims, observables the core does not represent) -- 13/23 once pH and water activity are set aside, and 4/5 on pH and water activity themselves, 0 of the misses being identical predictions across an axis the lane carries no term for. A coin scores ~0.5 on binary orderings; read the per-axis rows below, not the aggregate.
...
| Absolute concentration interval (90% CI) | every lane with sampled uncertainty | 5/33 evaluable literature rows inside; 5/32 out of sample; 6 not evaluable<br/><sub>results/validation/core_prediction_uncertainty.json (n=200); the no lane has no sampled uncertainty</sub> | **do-not-use** |
...
| Direction / ranking on `temperature` | any (temperature moved, everything else held) | 5/9 on the directional panel (independent claims)<br/><sub>misses: TEMP-01, TEMP-05, YIL-01, YIL-02</sub> | **do-not-use** |
