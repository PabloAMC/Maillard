...
| | kinetic core |
| --- | --- |
| rows within 3x of the measurement | **9 of 44** (median fold error 10.6x, geometric mean 19x) |
| **out of sample** — every row a core fit read removed | **8 of 43** (median 12x); since the primary-evidence refit only one scored row is a fit row |
| by lane, within 3x | acrylamide 2/12 · sulfur 2/19 · lipid 3/7 · trunk 2/6 |
...
| literature rows inside the 90% Monte-Carlo interval | **11 of 41** (median width 0.91 dex); **11 of 40** out of sample; 3 rows not evaluable — the 3-deoxyglucosone limb has no prior rows, so the envelope publishes those three as exact |
...
- **Absolute concentrations are unreliable.** On the union panel the kinetic core lands 9/44 rows within 3x (median fold error 10.6x, worst 9.63e+03x); out of sample -- every row a core fit read removed -- 8/43 (median 11.9x). Nothing in this repository licenses a ppb number as a specification. The core's 90% Monte-Carlo interval covers 11/41 evaluable literature rows (3 not evaluable: the no lane carries no sampled uncertainty), 11/40 out of sample.
...
| Absolute concentration interval (90% CI) | every lane with sampled uncertainty | 11/41 evaluable literature rows inside; 11/40 out of sample; 3 not evaluable<br/><sub>results/validation/core_prediction_uncertainty.json (n=200); the no lane has no sampled uncertainty</sub> | **do-not-use** |
...
| Any claim of benchmark-grade agreement | the union panel: trust loop + hold-outs + matrix bundles | 0/37 strict-ready (none); 9/44 rows within 3x, out-of-sample 8/43<br/><sub>recomputed live; strict-ready is the repository's own passing bar</sub> | **do-not-use** |
