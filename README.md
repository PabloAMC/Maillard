...
- **Absolute concentrations are unreliable.** On the union panel the kinetic core lands 4/46 rows within 3x (median fold error 30.9x, worst 4.23e+04x); out of sample -- every row a core fit read removed -- 3/45 (median 32.3x). Nothing in this repository licenses a ppb number as a specification. The core's 90% Monte-Carlo interval covers 10/42 evaluable literature rows (0 not evaluable: the no lane carries no sampled uncertainty), 10/41 out of sample.
...
| Absolute concentration (ppb) | protein matrix, lipid-derived aldehydes [lipid lane] | 0/14 rows within 3x, median 200x<br/><sub>recomputed live on the union panel; an absolute is never trust by rule</sub> | **do-not-use** |
...
| Any claim of benchmark-grade agreement | the union panel: trust loop + hold-outs + matrix bundles | 0/37 strict-ready (none); 4/46 rows within 3x, out-of-sample 3/45<br/><sub>recomputed live; strict-ready is the repository's own passing bar</sub> | **do-not-use** |
