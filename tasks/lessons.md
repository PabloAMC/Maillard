...
- **README Framing**: When the user asks for newcomer-facing documentation, do not center the README on current status dashboards or embedded plots. Lead with the scientific background, the trust regimes, the decision workflows, and only link out to state/progress artifacts as supporting material.
- **README Backbone**: For this repository, the README should be structured around problem, solution, tool/workflows, and evidence/limits. Repository mechanics and command reference should support that narrative, not replace it.
...
- **Proactive README Cleanup**: If a documentation cleanup is an obvious continuation of the user-approved direction, implement it in the same pass instead of merely suggesting it for later.
...
- **Root-Cause First Planning**: When several backlog gaps remain, prefer the shared blocker over the next easiest family closure. In this repo, a missing evidence contract for extrusion-conditioned external closure is a higher-value root cause than another isolated visibility upgrade.
