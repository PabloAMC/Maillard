...
- [ ] **The deep-research ledger is stale against its own generator** (found 2026-09-03 by accident: a bash script edited
      ...
      file is the version the literature tests and `test_data_headline_guards` census pin. Decide which is right, then
      add the ledger to `artifact_freshness_gate` (it is cheap) so it cannot drift again. Lesson recorded in
      `scripts/docker_maillard.sh`: do not edit the script while a `run` is in flight.
