...
| Kinetic core, scored on its own *(2026-09-03, retirement B3; the lane that replaces the one above)* | 38 bundles / 39 rows (2026-09-04: two sourceless bundles quarantined, eight hexose-only thiol rows declared NOT EVALUABLE by the engine's unidentified-entry rule): **4/39 within 3x**, out-of-sample **3/38**; envelope **6/32** evaluable literature rows inside the 90% CI, **6/31** out of sample (10/44 before the quantification classes were declared on 2026-09-03; one band-only interval is now not evaluable) (the B9 Laplace covariance, 20/23 identified, samples the sulfur lane; 5 rows not evaluable); **strict-ready 0/38** (`thiamine_cys_glucose_120C_Bolton1994` passed on assumed loadings; read in full on 2026-09-04 the core overpredicts its MFT 20x). Since wave B9 (2026-09-03) the sulfur fit reads primary evidence only: 6 panel rows over 6 step-level bundles, declared in `kinetic_core_b9_fit_targets.json`; the Hofmann 1998 level rows (ribose, xylose, glucose, fructose) are validation, and the xylose bundle is back on the hold-out panel. Without those rows the core predicts zero MFT from glucose and fructose. `results/validation/core_panel_scores.md`, `core_prediction_uncertainty.md` |
