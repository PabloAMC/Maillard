...
FAST_BARRIERS: Dict[str, Tuple[float, str]] = {
    ...
    # ── WAVE X TIER (added 2026-08-28) ───────────────────────────────────
    "furanone_reductive_sulfhydrylation": (28.60, "ESTIMATED, PENDING THE WAVE X FIT. norfuraneol + H2S + 2[H] -> 2-methyl-3-furanthiol + 2 H2O (C5H6O3 + H2S + H2 -> C5H6OS + 2 H2O, exact). The mechanism is Hofmann & Schieberle 1998's own Figure 1 (10.1021/jf9705983, p.238: 'Incorporation of a thiol group at carbon 3 yields 4-mercapto-5-methyl-3(2H)-furanone in a first reaction step, which in turn might be reduced by a reductone or by a further molecule of hydrogen sulfide. Elimination of water ... would give rise to MFT'); van den Ouweland & Peer 1975 (10.1021/jf60199a045) and Whitfield et al. 1993 report the same synthesis. SHIPPED VALUE 28.60 is the un-fitted `thiol_addition` CLASS value, the same neutral starting point Wave N gave `thiol_addition_pentodiulose`. IT IS AN ESTIMATE AND NOT A FIT, AND THAT IS THE INTERESTING PART: the fit against Hofmann Table 4 WAS run (scripts/generators/fit_furanone_reductive_sulfhydrylation_hofmann.py; record results/validation/furanone_reductive_sulfhydrylation_refit_hofmann.{json,md}) and was REJECTED BY ITS OWN PRE-REGISTERED ISOTOPE GATE. The Table 4 objective falls monotonically to the range floor and its indifference band's conservative edge is 26.30 kcal/mol; at 26.30 the norfuraneol channel supplies 58.1% of predicted MFT in the RIBOSE/CYSTEINE system, i.e. a MAJORITY, which is exactly what Cerny & Davidek 2003 exclude ('the resulting 2-methyl-3-furanthiol was mainly 13C5-labeled ... 4-hydroxy-5-methyl-3(2H)-furanone is unimportant as an intermediate'). THE TWO MEASUREMENTS ADMIT DISJOINT SETS: Table 4 wants <= 23.30 and the isotope ceiling admits nothing below 27.80, so no value of this constant satisfies both, and the model reproduces the fed-norfuraneol yield only by contradicting the labelling experiment. At the shipped 28.60 the in-situ share is 34.3% -- inside the 0.50 ceiling the source's word 'mainly' supports, but NOT comfortably, and the fit target is still 2.3x under-predicted. Do not read 28.60 as a calibrated number; read it as the value that was already there when the fit declined to move it. THIS KEY IS DELIBERATELY NOT `thiol_addition_norfuraneol` AND MUST NEVER INHERIT ITS 26.85. That constant was fitted (Wave H) THROUGH the route Wave N retired on isotope evidence AND against MFT 342 / FFT 200 ppb, which Wave S2b proved to be a repo-internal derivation from data/benchmarks/maillard_validation_benchmarks.md section 1.3 rather than a value from 10.1021/jf9705983 -- Wave W then confirmed from the primary source that 342 and 200 appear NOWHERE in the paper. A constant carrying two independent defects transfers no information; see the `thiol_addition_norfuraneol` entry above for its full history. THE STEP IS A SLOW PARALLEL CHANNEL, NOT A REPLACEMENT FOR THE 1,4-DIDEOXYOSONE ROUTE: Cerny & Davidek 2003 (10.1021/jf026123f) established by spiking that norfuraneol is unimportant as an IN-SITU intermediate, and that constraint is enforced as a regression test (tests/scientific/test_wave_x_step_level_2026_08.py::test_norfuraneol_route_stays_a_minor_share_of_ribose_mft_flux), not by the step's absence"),
    "furanone_amino_acid_reduction": (28.0, "ESTIMATED 2026-08-27 (Wave P item 6). hexose 1-deoxyosone + amino acid -> DMHF + Strecker aldehyde + CO2 + NH3 + H2O. This key REPLACES the routing of the hexose DMHF step through `furanone_cyclisation`, and is given the IDENTICAL value 28.0 so that any movement in predicted furaneol is attributable to the stoichiometry change (the `[HH]` pool gate was removed) and NOT to a barrier change. CITATION CORRECTED 2026-08-29 (Wave Q1): this entry previously read 'Blank & Fay 1996 ... and Kerler et al. 2010 ... name the amino acid as the reductant'. BLANK & FAY 1996 DO NOT SAY THAT. They leave the reductant explicitly OPEN: 'The reduction may occur either by a dismutation or by a reaction with further enoloxo compounds' (10.1021/jf950439o p. 534, after Schieberle 1992), and their figure legend calls the mechanism 'Hypothetical'. In that paper the amino acid's demonstrated role is as the Strecker-aldehyde/CARBON donor, not a hydride donor. The amino-acid coupling this key encodes rests on KERLER et al. 2010 (10.1002/9781444317770.ch3) ALONE, and on a weaker statement than 'names the reductant': Furaneol yield from acetylformoin 'was significantly enhanced in the presence of reductones such as ascorbic acid or methylene reductinic acid as well as the Strecker-active amino acid proline' -- an ENHANCEMENT observation that puts proline alongside genuine reductones. So the coupling is a DECLARED MODELLING CHOICE consistent with Kerler, not a mechanism any cited paper asserts. Blank & Fay 1996 remains a correct citation for the route TOPOLOGY (Amadori -> 2,3-enolisation -> Strecker chain elongation -> acetylformoin -> reduction) and for the amino acid as carbon donor. Resolving the reductant properly requires Hofmann & Schieberle 2001, 'Acetylformoin -- an important progenitor', Flavour 2000 Proc. pp. 311-322, which both Wang & Ho 2008 and Poisson 2019 cite for it and which this repo does not hold (data/lit/extraction_dossiers/k5b_dmhf_synthesis.md row R6). Same `dehydration`/`furanone_formation` analogue as its sibling `furanone_formation` (28.0). UNCONSTRAINED"),
