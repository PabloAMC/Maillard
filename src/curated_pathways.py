...
draws its figure FROM THIS FILE, so that figure depicts PRE-WAVE-P CHEMISTRY: it
shows neither the C2+C3 recombination lane to MFT nor fructose's independent HMF
route.  It is a schematic of the curated layer, not a picture of the shipped
network.

RESOLVED 2026-08-29 (Wave Q1) by removing the publication rather than the
warning.  `docs/assets/reaction_network.pdf` was DELETED and the script no longer
copies into `docs/assets/`.  A published figure that needs a paragraph explaining
which chemistry it omits is worse than no published figure, and the omission was
invisible to anyone who opened the PDF without reading this file.  The published
network map is now `docs/assets/network_map.html`, generated by
`scripts/generators/generate_network_map.py` FROM `src/kinetic_core/` -- i.e.
from the shipped network itself, so it cannot drift from it the way this one
did.  `scripts/generate_reaction_network.py` still writes into
`results/validation/` for anyone who wants the curated-layer schematic; it is
just not published as documentation any more.
