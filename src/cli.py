"""
The front door: `maillard <verb>`. Installed as the `maillard` command by `pip install -e .`
(pyproject.toml [project.scripts]); `scripts/maillard.py` is a shim onto this module for the
container. Verbs: compare, predict, explain, score, calibrate, rank, wishlist, ui.
"""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    ui = verbs.add_parser(
        "ui",
        help="a local page in your browser: paste a spec, get the report (no account, no server elsewhere)",
        description=(
            "Serve one page on this machine (http://127.0.0.1:8765 by default) that takes a compare or\n"
            "predict spec, runs it through the same code the verbs use, and returns the HTML report.\n"
            "Standard library only; nothing leaves your computer."
        ),
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    ui.add_argument("--port", type=int, default=8765)
    ui.add_argument("--no-browser", action="store_true", help="do not open the page automatically")
