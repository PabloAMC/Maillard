...
def _spec(aw=None, name="pot", temp=160.0):
    # pH 6.8 = the lane's reference, so only the a_w terms speak here (B15 added a pH factor)
    return FormulationSpec(name=name, precursors={"L-Asparagine": 10.0, "D-Glucose": 10.0},
                           process=ProcessSpec(thermal=ThermalProgram.isothermal(temp, 20.0), ph=6.8, water_activity=aw))


def test_the_band_and_window_are_the_source_table():
    assert ac.AW_WINDOW == (0.34, 0.99)          # B15 extended the 2008 window (0.88-0.99) down with the 2007 series
    ...
    assert ac.aw_multiplier(None) == 1.0 and ac.aw_multiplier(0.2) == 1.0 and ac.aw_multiplier(0.95) == 1.0
    assert ac.aw_multiplier(0.95, 0.7) == 0.7 and ac.aw_multiplier(0.2, 0.7) == 1.0   # the draw acts only inside
...
def test_declarations_name_the_window_state():
    ...
    outside = ac.declarations(_spec(0.20).process)
...
def test_a_prediction_at_the_centre_is_unchanged_and_the_draw_moves_it():
    ...
    assert 0 < low < inside
    outside = predict(_spec(0.2), [ACR], draw=CoreDraw(acrylamide_aw_scale=0.5)).concentrations_ug_per_l[ACR]
...
def test_the_axis_is_answered_inside_the_window_and_refused_across_it():
    ...
    assert reason is not None and "measured only inside 0.34-0.99" in reason
    # the envelope declaration prints the term, not the old "metadata only" line
    assert any("WATER ACTIVITY (B14/B15)" in w for w in da.warnings)
