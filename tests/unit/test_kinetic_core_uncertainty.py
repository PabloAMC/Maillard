...
def test_sample_draws_is_deterministic_and_seed_sensitive():
    ...
    for d in a:
        ...
        from src.kinetic_core.ph_state import PhDrift
        assert isinstance(d.core.ph_drift, PhDrift)  # B8: drawn from the Laplace covariance
        assert "log10_k_ref_at_145C" in (d.core.maillard or {})  # B8: sulfur is sampled from its covariance
...
def test_extraction_rows_do_not_carry_the_headspace_bands(small_artifact):
    ...
    # B8: the sulfur lane is sampled from its Laplace covariance, so an extraction row
