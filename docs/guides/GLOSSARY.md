...
| **B10** | the temperature-structure attempt: the lumped formation barrier split in two, Yiltirak 2026's within-study folds as fit rows. Refused (neither barrier identified); only its ambient-oxidant consistency fix shipped | `kinetic_core_b10_prereg.md`, `kinetic_core_b10_ship_rule.md` |
| **B11** | oxygen as an input: a two-pool headspace reservoir and two consumers. Refused (consumers unidentified); the state ships inert | `kinetic_core_b11_prereg.md`, `kinetic_core_b11_ship_rule.md` |
| **B12, B13** | not fits: declared water-activity and pH terms on the trunk (B12) and the trunk-only dicarbonyl species (B13) | `kinetic_core_b12_prereg.md`, `src/kinetic_core/trunk_conditions.py` |
| **B14, B15** | not fits: the acrylamide lane's declared water-activity term and window (B14) and pH factor (B15) | `kinetic_core_b14_prereg.md`, `src/kinetic_core/acrylamide_conditions.py` |
| **B16** | the thiol-sink re-tuning on the 100 °C series and the TTCA decay rows. Refused; its diagnosis is the introduction's section 7 | `kinetic_core_b16_prereg.md`, `kinetic_core_b16_ship_rule.md` |
| **B17** | the sink structure. Variant (b): the thiol disulfide made reversible on one shared release constant; refused, the data drove the release to zero and showed the disulfide channel is oxidant-limited. Variant (a): a saturating adduct on an electrophile pool the pot makes, feeding the measured binding step; refused, the pool's source is switched off at the optimum and the measured equilibrium releases the thiol when hot. A variant (c), irreversible addition on a pool the lane's live flux makes, is the next to pre-register | `kinetic_core_b17_prereg.md` (sections 6 and 6b), `kinetic_core_b17_ship_rule.md`, `kinetic_core_b17a_ship_rule.md` |
| **B18** | a pyrazine step on the sugar path: two Strecker constants fitted on fed-dicarbonyl ladders, a two-slope pH term, the condensation declared fast. Ships, with the caveat that the sugar-pot supply of the small dicarbonyls is not measured | `kinetic_core_b18_prereg.md`, `kinetic_core_b18_ship_rule.md`, `parameters_pyrazine.py` |
| **B20** | the glycation arm: the protein's bound lysine as a reactant on the sugar path, to the bound Amadori compound, CML and CEL; rates from one laboratory (casein in water), barriers declared. Ships; answers only on a pot with a stated protein loading | `kinetic_core_b20_prereg.md`, `kinetic_core_b20_ship_rule.md`, `parameters_glycation.py` |
| **B21** | the aqueous glucosone route to glyoxal: the Amadori compound gives glucosone in water at one laboratory's rate, replacing the dry-glass entry as the operative supply. Ships; a sugar and amine pot now holds glyoxal at the level measured; the pyrazine total's miss remains and is no longer the glyoxal | `kinetic_core_b21_prereg.md`, `kinetic_core_b21_ship_rule.md`, `parameters_dicarbonyl.py` |
| **B22** | the methionine chain on the sugar path: methional as methionine's Strecker aldehyde on the free dicarbonyls at a ratio to glycine, then methanethiol and the disulfide. Refused: the ratio ran to its ceiling with one laboratory's rates still four decades low and another's two decades high; the route is the methionine Amadori compound's own decay, not the free dicarbonyls | `kinetic_core_b22_prereg.md`, `kinetic_core_b22_ship_rule.md`, `parameters_methionine.py` |
| **B24** | 2-acetyl-1-pyrroline from proline: the Strecker of proline to 1-pyrroline and its acylation by methylglyoxal, fitted on one laboratory's fed yields. Refused: the fed acylation fits, the chain from proline saturates in the source and not in the arm, for want of the tetrahydropyridine branch and a pyrroline loss | `kinetic_core_b24_prereg.md`, `kinetic_core_b24_ship_rule.md`, `parameters_proline.py` |
| **B25** | the thiol sink, variant (c): the meaty thiols' irreversible addition to the pot's own deoxypentosones. Refused: the fit switched the step off at every temperature; the third sink structure refused on the same rows, which points at the objective's weighting or the laboratory, not at a fourth structure | `kinetic_core_b25_prereg.md`, `kinetic_core_b25_ship_rule.md` |
| **S, K, Q, V, W waves** (e.g. Wave S2c, K6a, Q1, V1, W) | the retired legacy lane's audit and repair waves, August 2026 | `docs/history/AUDIT_legacy_lane_2026-08.md`, `docs/history/` |
