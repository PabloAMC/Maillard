> **ARCHIVED 2026-09-03 (step B7).** This report scored the RETIRED screening lane. The kinetic core's
> directional scorecard is `results/validation/core_directional_scores.md` (generated by
> `scripts/generators/generate_core_directional_scores.py` from the same panel); the CLI tags and the
> model card read that artifact, not this file. Kept verbatim as history.
