...
This report defines and validates the minimum external evidence package for unlocking a meaty-positive matrix benchmark for pea protein isolate (PPI). *(2026-08-27, Wave R: this sentence used to add "as specified in `docs/EXTERNAL_MATRIX_BENCHMARK_UNLOCK_REPORT.md`". That document is absent from the repository and from `git ls-files`, so this protocol implements a specification that exists nowhere. The dangling pointer is removed rather than invented; the requirements this document actually works to are the ones stated in its own sections below.)* The core scientific challenge is that the pea matrix presents a structural asymmetry: its off-flavour profile (hexanal, 2-pentylfuran) is well-documented in quantitative external literature, but its meaty-positive volatile output under controlled precursor augmentation is absent from the benchmark set. We demonstrate, using published model-system kinetics and matrix-specific headspace data, that a narrow exogenous ribose + cysteine system at pH 5.5–6.0 and 95–120°C is both chemically justified and experimentally tractable as the first meaty-positive unlock benchmark for PPI.
