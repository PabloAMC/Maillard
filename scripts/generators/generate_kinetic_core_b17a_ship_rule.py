#!/usr/bin/env python
"""
Wave B17 variant (a) -- THE PRE-REGISTERED SHIP RULE, evaluated (2026-09-09).

`results/validation/kinetic_core_b17_prereg.md` sec. 4, computed from the frozen B9 and B17a artifacts
WITHOUT switching the engine (the candidate vector is handed to `core_parameters(SULFUR, frozen=...)`),
written to `results/validation/kinetic_core_b17a_ship_rule.{json,md}`. Same six tests as variant (b);
T6 is on the site yield. T3's Zhang 2024 share is reported as FIGURE-DERIVED (`zhang2024b_extraction.md`:
the printed text supports only an ordering) and only Zhou 2023's shares decide T3.
  ...
  T6  identification: Laplace sigma on log10 site yield below one decade, off its bound; the cost slice
...
B17_REPORT = V / "kinetic_core_b17a_fit_report.json"
B17_LAPLACE = V / "kinetic_core_b17a_laplace_covariance.json"
OUT = V / "kinetic_core_b17a_ship_rule.json"
...
def _operative_and_drift(report, with_release: bool):
    ...
    if with_release and fr.get("mele_site_log10_yield"):
        block["mele_site_log10_yield"] = dict(fr["mele_site_log10_yield"])
...
def t2_b9_rows(rep, b9, sigma):
    ...
    for rid, v9 in r9.items():
        if rid in resid and rid in sigma:
            growth.append({"row": rid, "b9_residual": float(v9), "b17a_residual": float(resid[rid]),
...
def t6_identification(rep, lap, B17):
    ...
    x = float(fr["mele_site_log10_yield"]["mele_site_yield"])
    lo, hi = rep["site_yield_band_log10"]
    on_bound = bool(x - lo <= 1e-3 * (hi - lo) or hi - x <= 1e-3 * (hi - lo))
    out: Dict[str, Any] = {"log10_mele_site_yield": x, "band": [lo, hi], "on_bound": on_bound,
                           "k_mele_site_per_min_at_145C": fr["mele_site"]["k_mele_site_per_min_at_145C"]}
    if lap:
        ...
        out["sigma_dex"] = sig.get("mele_site_yield")
        out["identified"] = ident.get("mele_site_yield")
        out["n_identified"] = sum(ident.values())
        out["n_free"] = len(names)
    # the cost slice along the yield coordinate (quick mode), the profile the prereg asks for
    ...
    for d in (-1.0, -0.5, 0.0, 0.5, 1.0):
        xx = x_opt.copy()
        xx[B17.YIELD_SLOT] = min(max(x + d, lo), hi)
...
def render(p: Dict[str, Any]) -> str:
    ...
    L = [f"# Wave B17 variant (a) ship rule: {p['verdict']}", "", f"*Rule: {p['rule']}. Pre-registration `{p['prereg']}` sec. 2 (a).*", "",
         f"log10 electrophile-site yield per osone decayed: {t6['log10_mele_site_yield']:.3f} (band {t6['band']}; k_mele_site {t6['k_mele_site_per_min_at_145C']:.3g} /min at 145 C); cost {p['cost']:.2f}; active bounds {p['active_bounds'] or 'none'}", "",
         "| test | result | pass |", "|---|---|---|",
...
def main() -> int:
    ...
    import generate_kinetic_core_b17a_fit as B17  # noqa: E402  (configure() at import)
    ...
    payload = {
        "artifact": "kinetic_core_b17a_ship_rule",
        "provenance": provenance.provenance_block("kinetic_core_b17a_ship_rule", generated_by="scripts/generators/generate_kinetic_core_b17a_ship_rule.py",
                                                  inputs=[p for p in (B9_REPORT, B17_REPORT, B17_LAPLACE) if p.exists()]),
        "prereg": data_paths.rel(V / "kinetic_core_b17_prereg.md"),
        "rule": "SHIP if T1 (reference pot ratios within 0.5 dex and still rising 6 -> 12 h), T2 (no B9 row +0.3 dex) and T6 (site yield identified, off its bound, slice not flat) hold; T3-T5 reported",
