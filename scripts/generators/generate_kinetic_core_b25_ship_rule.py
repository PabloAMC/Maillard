#!/usr/bin/env python
"""
Wave B25 -- THE PRE-REGISTERED SHIP RULE, evaluated (2026-09-09).

`results/validation/kinetic_core_b25_prereg.md` sec. 4 (B17's six tests), computed from the frozen B9 and B25
artifacts WITHOUT switching the engine (the candidate vector is handed to `core_parameters(SULFUR, frozen=...)`),
written to `results/validation/kinetic_core_b25_ship_rule.{json,md}`. T6 is on the addition constant and its
barrier; T3's Zhang 2024 share is reported as figure-derived and only Zhou 2023's shares decide T3.
  ...
  T6  identification: Laplace sigma on log10 k_add below one decade and on Ea_add below 60 kJ/mol, neither on
      its bound; the cost slices along the coordinates not flat
...
B17_REPORT = V / "kinetic_core_b25_fit_report.json"
B17_LAPLACE = V / "kinetic_core_b25_laplace_covariance.json"
OUT = V / "kinetic_core_b25_ship_rule.json"
...
#: Zhang 2024 Fig. 1b (Cys arm), a 300-dpi READ-OFF in the older dossier: MFT-MFT / MFT mass ratio 0.086 -> molar dimer/monomer
#: x 114.17/226.32, x2 thiol equivalents. FIGURE-DERIVED (zhang2024b_extraction.md); reported, not a pass criterion.
...
def _operative_and_drift(report, with_release: bool):
    ...
    if with_release and fr.get("thiol_addition"):
        block["thiol_addition"] = dict(fr["thiol_addition"])
...
def t2_b9_rows(rep, b9, sigma):
    ...
    for rid, v9 in r9.items():
        if rid in resid and rid in sigma:
            growth.append({"row": rid, "b9_residual": float(v9), "b25_residual": float(resid[rid]),
...
def t3_dimer_shares(op, drift):
    ...
    out["worst_dex"] = worst
    out["zhang2024"]["evidence"] = "figure_derived (zhang2024b_extraction.md); reported only"
    worst_zhou = max((abs(r["dex"]) for r in out["zhou2023"].values() if "dex" in r), default=float("inf"))
    out["worst_dex_zhou"] = worst_zhou
    out["pass"] = bool(worst_zhou <= 0.3 and all("dex" in r for r in out["zhou2023"].values()))
    return out
...
def t6_identification(rep, lap, B17):
    ...
    xk = float(fr["thiol_addition"]["log10_k_add_145C"])
    xe = float(fr["thiol_addition"]["ea_add_kj_mol"])
    lok, hik = rep["addition_band_log10k"]
    loe, hie = rep["addition_band_ea"]
    on_bound_k = bool(xk - lok <= 1e-3 * (hik - lok) or hik - xk <= 1e-3 * (hik - lok))
    on_bound_e = bool(xe - loe <= 1e-3 * (hie - loe) or hie - xe <= 1e-3 * (hie - loe))
    out: Dict[str, Any] = {"log10_k_add_145C": xk, "ea_add_kj_mol": xe, "band_log10k": [lok, hik], "band_ea": [loe, hie],
                           "on_bound": {"log10_k_add": on_bound_k, "Ea_add": on_bound_e}}
    if lap:
        ...
        out["sigma_dex"] = sig.get("log10_k_add_145C")
        out["sigma_ea"] = sig.get("ea_add_kj_mol")
        out["identified"] = {"log10_k_add": ident.get("log10_k_add_145C"), "Ea_add": ident.get("ea_add_kj_mol")}
    ...
    slices = {}
    for name, slot, x0, lo, hi, steps in (("log10_k_add", B17.K_SLOT, xk, lok, hik, (-1.0, -0.5, 0.0, 0.5, 1.0)),
                                          ("Ea_add", B17.EA_SLOT, xe, loe, hie, (-30.0, -15.0, 0.0, 15.0, 30.0))):
        pts = {}
        for d in steps:
            xx = x_opt.copy()
            xx[slot] = min(max(x0 + d, lo), hi)
            r = B17.residual_vector(xx, True)
            pts[f"{d:+.1f}"] = 0.5 * float(np.dot(r, r))
        c0 = pts["+0.0"]
        verdict = ("bound_limited" if out["on_bound"][name] else ("flat" if max(pts.values()) - c0 < 0.5 else
                   ("quadratic" if list(pts.values())[0] > c0 and list(pts.values())[-1] > c0 else "shifted")))
        slices[name] = {"cost_slice": pts, "verdict": verdict}
    out["slices"] = slices
    out["pass"] = bool((out.get("sigma_dex") is not None and out["sigma_dex"] < 1.0) and (out.get("sigma_ea") is not None and out["sigma_ea"] < 60.0)
                       and not on_bound_k and not on_bound_e and all(s["verdict"] not in ("bound_limited", "flat") for s in slices.values()))
    return out


def render(p: Dict[str, Any]) -> str:
    t1, t2, t3, t4, t5, t6 = p["T1"], p["T2"], p["T3"], p["T4"], p["T5"], p["T6"]
    L = [f"# Wave B25 ship rule: {p['verdict']}", "", f"*Rule: {p['rule']}. Pre-registration `{p['prereg']}`.*", "",
         f"log10 k_add at 145 C: {t6['log10_k_add_145C']:.3f} (band {t6['band_log10k']}); Ea_add {t6['ea_add_kj_mol']:.1f} kJ/mol (band {t6['band_ea']}); cost {p['cost']:.2f}; active bounds {p['active_bounds'] or 'none'}", "",
         "| test | result | pass |", "|---|---|---|",
         ...
         f"| T3 dimer shares | Zhou 2023: {{ {', '.join(f'pH {k}: model {v.get('model_mft_share_pct', float('nan')):.2f} % vs {v.get('zhou_mft_share_pct')} %' for k, v in t3['zhou2023'].items())} }}; Zhang 2024 Cys arm (figure-derived, reported only): model {t3['zhang2024'].get('model_mft_share_pct', float('nan')):.2f} % vs {t3['zhang2024']['zhang_mft_share_pct']:.1f} %; worst (Zhou) {t3['worst_dex_zhou']:.2f} dex | {t3['pass']} |",
         ...
         f"| T6 identification | sigma {t6.get('sigma_dex')} dex / {t6.get('sigma_ea')} kJ/mol, identified {t6.get('identified')}, on bound {t6['on_bound']}, slices {json.dumps({k: v['verdict'] for k, v in t6['slices'].items()})} | {t6['pass']} |",
...
def main() -> int:
    ...
    import generate_kinetic_core_b25_fit as B17  # noqa: E402  (configure() at import)
    ...
    payload = {
        "artifact": "kinetic_core_b25_ship_rule",
        "provenance": provenance.provenance_block("kinetic_core_b25_ship_rule", generated_by="scripts/generators/generate_kinetic_core_b25_ship_rule.py",
                                                  inputs=[p for p in (B9_REPORT, B17_REPORT, B17_LAPLACE) if p.exists()]),
        "prereg": data_paths.rel(V / "kinetic_core_b25_prereg.md"),
        "rule": "SHIP if T1 (reference pot ratios within 0.5 dex and still rising 6 -> 12 h), T2 (no B9 row +0.3 dex) and T6 (the addition constant and its barrier identified, off their bounds, slices not flat) hold; T3-T5 reported",
