#!/usr/bin/env python
"""
B41 ship rule (2026-09-11), judged on FROZEN artifacts: the B41 fit report, and the panel
scorecard BEFORE (results/validation/_b41_baseline/core_panel_scores_before_b41.json, the tracked
panel at commit c365533, before the fit) and AFTER (.../core_panel_scores_after_b41.json, the panel
...
Run inside docker:  PYTHONPATH=/workspace python scripts/generators/generate_kinetic_core_b41_ship_rule.py
"""
...
PREREG = VAL / "kinetic_core_b41_prereg.md"
FIT = VAL / "kinetic_core_b41_fit_report.json"
BEFORE = VAL / "_b41_baseline" / "core_panel_scores_before_b41.json"
AFTER = VAL / "_b41_baseline" / "core_panel_scores_after_b41.json"
OUT_JSON = VAL / "kinetic_core_b41_ship_rule.json"
OUT_MD = VAL / "kinetic_core_b41_ship_rule.md"
...
def main() -> int:
    ...
    mgo_b, mgo_a = fe(rb, "methylglyoxal"), fe(ra, "methylglyoxal")
    mgo_ok = mgo_a is not None and mgo_a <= max(mgo_b or 1e9, 3.0) * 1.03
    p4["methylglyoxal"] = (mgo_b, mgo_a)
    ...
    ships = p1 and p4_pass and p6_pass and tdg_ok and mgo_ok
    counts = {"before": {"within": len(within_before), "rows": len(rb)}, "after": {"within": len(within_after), "rows": len(ra)}}
    payload = {
        "artifact": "kinetic_core_b41_ship_rule",
        "provenance": provenance.provenance_block("kinetic_core_b41_ship_rule", generated_by="scripts/generators/generate_kinetic_core_b41_ship_rule.py",
                                                  wave="B41", inputs=[PREREG, FIT, BEFORE, AFTER]),
        "rule": "SHIP if P1, P2 (Leitzen: 3,4-DGE < 10x, 3-DG <= 3x, HMF within 3 %, METHYLGLYOXAL within 3 % of 1.28x) and P3 (no row leaves the band); P4 reported (prereg sec. 5)",
        "P1": pr["P1_rows_fit_with_tmax"], "P2": pr["P2_k_tdg_ddg_moves_less_than_b39"], "P2_methylglyoxal_row": {"fold_before_after": (mgo_b, mgo_a), "rule": "within 3 % of before or <= 3x", "pass": mgo_ok}, "P2_3dg_row": {"fold_before_after": p4["3-deoxyglucosone"], "pass": tdg_ok},
    ...
    L = [f"# Wave B41 ship rule: {payload['verdict']}", "", f"_{payload['rule']}. Judged on frozen artifacts: the fit report and the panel before/after._", "",
         ...
         f"- **P2 decisive rows**: 3-DG ≤ 3× **{'HELD' if tdg_ok else 'REFUTED'}** {p4['3-deoxyglucosone']}; methylglyoxal within 3 % **{'HELD' if mgo_ok else 'REFUTED'}** {(mgo_b, mgo_a)}",
    ...
    OUT_MD.write_text("\n".join(L) + "\n")
    print(payload["verdict"], "| P1", p1, "| 3-DG<=3x", tdg_ok, "| MGO ok", mgo_ok, "| P4", p4_pass, {k: (None if b is None else round(b, 2), None if a is None else round(a, 2)) for k, (b, a) in p4.items()},
