#!/usr/bin/env python
"""
Build Wave B17 -- THE SINK STRUCTURE, variant (b): THE DISULFIDE GIVES THE THIOL BACK (2026-09-08).

Pre-registered in ``results/validation/kinetic_core_b17_prereg.md``. B16's objective unchanged (B9's
54 rows + Schieberle 2000's seven within-study ratios at 100 C + Zhai 2021's three TTCA rows = 64
rows; every B9 band kept, the thiol-sink ceiling at 102 kJ/mol) and ONE new coordinate appended to
the vector, the way B11 appended its oxygen consumers: ``log10_k_dimer_release``, the shared rate of
``ch_dimer_release_mft`` / ``ch_dimer_release_fft`` (MFTD -> 2 MFT, FFTD -> 2 FFT; sulfur.py), whose
barrier is the dimerisation's own measured one (Zhang 2026 k17, 122.2 kJ/mol), so the equilibrium
constant carries the temperature dependence. 24 free. Start 0 is B9's optimum with the release at
the centre of its band; start 1 is B8's perturbation protocol. Two starts, the 600-evaluation
budget, quick mode for the search.

Usage (each start ~30 min; run both in parallel, then consolidate):
    python scripts/generators/generate_kinetic_core_b17_fit.py --start 0
    python scripts/generators/generate_kinetic_core_b17_fit.py --start 1
    python scripts/generators/generate_kinetic_core_b17_fit.py --consolidate
"""
...
from src.kinetic_core.parameters_sulfur import (  # noqa: E402
    DIMER_RELEASE_BOUNDS_LOG10K, MEASURED_SULFUR, ZHANG_EA_THIOL_TO_DISULFIDE_KJ_MOL,
    dimer_release_parameters, with_fitted_sulfur,
)
...
MEMBER_DIR = data_paths.VALIDATION_DIR / "kinetic_core_b17_members"
OUT_FIT_REPORT = data_paths.VALIDATION_DIR / "kinetic_core_b17_fit_report.json"

# B16's bindings, captured before configure() rebinds B8's names to B17's.
B16.configure(False)
...
# THE VECTOR: 48 + 1
# ===========================================================================
ALL_KEYS: Tuple[str, ...] = tuple(_B8_ALL_KEYS) + ("log10_k_dimer_release",)
RELEASE_SLOT = len(ALL_KEYS) - 1
FREE_KEYS: Tuple[str, ...] = tuple(_B8_FREE_KEYS) + ("log10_k_dimer_release",)
FREE_CLAUSE_OF: Dict[str, str] = {
    **dict(_B8_FREE_CLAUSE_OF),
    "log10_k_dimer_release": "B17 variant (b): the disulfide gives the thiol back; barrier shared with the dimerisation (Zhang 2026 k17)",
}
...
COORDINATE_OF_OVERRIDES: Dict[str, Dict[str, str]] = {
    "log10_k_dimer_release": {"block": "dimer_release_log10_k", "key": "k_dimer_release", "kind": "log10k"},
}
...
def full_bounds() -> Tuple[np.ndarray, np.ndarray]:
    ...
    return (np.append(np.array(lower, dtype=float), DIMER_RELEASE_BOUNDS_LOG10K[0]),
            np.append(np.array(upper, dtype=float), DIMER_RELEASE_BOUNDS_LOG10K[1]))


def release_from_vector(x: np.ndarray) -> float:
    return 10.0 ** float(x[RELEASE_SLOT])
...
def build_parameters(x: np.ndarray) -> Dict[str, Any]:
    ...
    parameters.update(dimer_release_parameters(k_dimer_release=release_from_vector(x)))
    return parameters


def incumbent_vector() -> np.ndarray:
    """B9's frozen optimum with the release constant at the centre of its band."""
    ...
    centre = 0.5 * (DIMER_RELEASE_BOUNDS_LOG10K[0] + DIMER_RELEASE_BOUNDS_LOG10K[1])
...
def consolidate() -> Dict[str, Any]:
    ...
    fr["dimer_release_log10_k"] = {"k_dimer_release": float(x[RELEASE_SLOT])}
    fr["dimer_release"] = {"k_dimer_release_per_min_at_145C": release_from_vector(x),
                           "ea_kj_mol_shared_with_dimerisation": ZHANG_EA_THIOL_TO_DISULFIDE_KJ_MOL}
    payload["wave"] = f"{WAVE} -- the sink structure, variant (b): the disulfide gives the thiol back"
    payload["generated_by"] = "scripts/generators/generate_kinetic_core_b17_fit.py"
    payload["prereg"] = data_paths.rel(PREREG)
    payload["declaration"] = "docs/reference/FIT_HOLDOUT_DECLARATION.md Amendment 28"
    payload["objective"]["form"] = "B16's 64 rows (B9's 54 + 7 Schieberle 2000 ratios + 3 Zhai 2021 TTCA rows); one release constant appended (24 free)"
    ...
    payload["release_band_log10k"] = list(DIMER_RELEASE_BOUNDS_LOG10K)
    payload["thiol_sink_ceiling_kj_mol"] = float(full_bounds()[1][ALL_KEYS.index("Ea_decay_thiol_sink")])
    payload["start_vector"] = "B9's frozen optimum with the release constant at the centre of its band (start 0); B8's perturbation protocol (start 1)"
    lower, upper = full_bounds()
    active = []
    ...
    print(f"rewrote {OUT_FIT_REPORT} as {WAVE}: log10 k_dimer_release {x[RELEASE_SLOT]:.3f}; active bounds: {[a['key'] for a in active]}")
    return payload
...
def main(argv=None) -> int:
    ...
    assert PREREG.exists(), "B17 is pre-registered; write the prereg before running the fit"
    configure()
    ...
    member["wave"] = WAVE
    member["log10_k_dimer_release"] = float(member["x_full"][RELEASE_SLOT])
    dest = MEMBER_DIR / f"b17_s{args.start}.json"
