...
def main() -> int:
    ...
    payload = build_wave1_dft_ingestion_artifact(
        manifest_payload=manifest_payload,
        execution_path=Path(args.execution),
    )

    markdown_path = output_dir / "wave1_dft_ingestion_report.md"
    json_path = output_dir / "wave1_dft_ingestion_report.json"
    markdown_path.write_text(render_wave1_dft_ingestion_markdown(payload), encoding="utf-8")
