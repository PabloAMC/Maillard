...
`results/validation/kinetic_core_b2_1_fit_report.json`, changes NOTHING, predicts
...
def load_frozen() -> Dict[str, Any]:
    if not FIT_REPORT.exists():
        raise SystemExit(
            f"{FIT_REPORT} not found. Run generate_kinetic_core_b2_1_fit.py first; "
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Score Build Wave B2.1's pre-registered hold-out rows against the "
            "frozen B2.1 parameters, fitting nothing; writes "
            "results/validation/kinetic_core_b2_1_holdout_report.{json,md}."
        )
    )
    parser.add_argument(
        "--output-dir",
    ...
    payload = {
        ...
        "generated_by": "scripts/generators/generate_kinetic_core_b2_1_holdout.py",
    ...
    # =====================================================================
    # B2's report is read, never written. Rows that B2 did not have (the Kang
    ...
    if b2_path.exists():
        ...
        for r in rows:
            ...
            comparison.append({
                "id": r["id"],
                "gating": r["gating"],
                "b2_pass": None if before is None else before["pass"],
                "b2_fold": None if before is None else before.get("fold_error"),
                "b2_1_pass": r["pass"],
                "b2_1_fold": r.get("fold_error"),
                "status": (
    ...
    payload["comparison_with_b2"] = {
        "b2_scorecard": b2_summary,
        "b2_1_scorecard": payload["scorecard"],
        "rows": comparison,
        "how_to_read_it": (
            ...
            "which names one B2 pass that its own report refused to count and "
            "which B2.1 expected to lose. Rows marked NEW ROW did not exist in "
            "B2 and were added by Amendments 4 and 5."
        ),
    }

    out_json = Path(args.output_dir) / "kinetic_core_b2_1_holdout_report.json"
    out_json.parent.mkdir(parents=True, exist_ok=True)
    out_json.write_text(json.dumps(payload, indent=2, default=str))

    lines: List[str] = []
    a = lines.append
    a("# Kinetic core, Build Wave B2.1 -- the pre-registered hold-out scorecard")
    a("")
    a("Predicted from the FROZEN fit in "
      "`results/validation/kinetic_core_b2_1_fit_report.json`. **No parameter was "
    ...
    if comparison:
        ...
        a(f"**Against B2: {b2_g[0]} / {b2_g[1]} gating.** "
          f"B2.1 adds {sum(1 for c in comparison if c['status'] == 'NEW ROW')} "
          f"rows the declaration's Amendments 4 and 5 created, so the "
          f"denominators differ and the two totals are NOT directly comparable; "
          f"the row-by-row table below is.")
        a("")
        a("## B2 versus B2.1, row by row")
        a("")
        a("| row | gating | B2 fold | B2 | B2.1 fold | B2.1 | |")
        a("|---|---|---:|---|---:|---|---|")
        for c in comparison:
            def _f(v):
            ...
            a(f"| `{c['id']}` | {'yes' if c['gating'] else 'no'} | "
              f"{_f(c['b2_fold'])} | {_p(c['b2_pass'])} | "
              f"{_f(c['b2_1_fold'])} | {_p(c['b2_1_pass'])} | {c['status']} |")
        a("")
        a(payload["comparison_with_b2"]["how_to_read_it"])
        a("")
    ...
    out_md = Path(args.output_dir) / "kinetic_core_b2_1_holdout_report.md"
    out_md.write_text("\n".join(lines))
