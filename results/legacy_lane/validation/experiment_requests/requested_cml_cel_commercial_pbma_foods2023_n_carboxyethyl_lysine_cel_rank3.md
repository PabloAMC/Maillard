# Experiment Request `requested_cml_cel_commercial_pbma_foods2023_n_carboxyethyl_lysine_cel_rank3`

VoI rank **#3**, score **4.03**.
Generated from the current Monte-Carlo benchmark envelope.

## Why this experiment

- Compound: **Nε-(Carboxyethyl)lysine (CEL)**
- Envelope miss: 3.03 dex
- 90% CI width: 0.00 dex
- ODT: unknown; decision relevance 1.00
- Rationale: measured outside 90% CI by 3.03 dex; CI width 0.00 dex; safety marker — needs SIDA-grade absolute anchor
...
- `internal_standards`:
  - `compound_specific_internal_standard_for_n_carboxyethyl_lysine_cel`

## CRO send-to-lab checklist

- [ ] Confirm target compound identity and calibrate over a range spanning BOTH the published value (≈ 55000 ppb) and the model's 90% CI midpoint (≈ 51.6752 ppb). They differ by 1.06e+03× — that disagreement is *why* this experiment is ranked, so a calibration range covering only one of them cannot resolve it.
...
- [ ] Record the measured `conc_ppb` and `uncertainty_pct` (1σ relative) under `measured_volatiles.Nε-(Carboxyethyl)lysine (CEL)` in the pre-filled intake YAML.
...
- Pre-filled intake YAML: `data/protocols/requested_cml_cel_commercial_pbma_foods2023_n_carboxyethyl_lysine_cel_rank3.yaml`
- Fill in `measured_volatiles.Nε-(Carboxyethyl)lysine (CEL).conc_ppb` (and any
