# Kinetic core, Build Wave B2.3 -- the pre-registered hold-out scorecard

Predicted from the FROZEN fit in `results/validation/kinetic_core_b2_3_fit_report.json`. **No parameter was changed after the fit**; there is no optimiser in the scoring script.

**GATING: 7 / 30 passed.** Diagnostic: 7 / 16. Unscoreable or explicitly deferred: 3.

**MEDIAN |log10 FOLD| ON THE GATING ROWS: 1.007** (geometric-mean fold 21.40x, n=29). All scored rows: 0.997 (16.59x, n=41).

> A pass/fail count is CENSORED: a row already outside the band can degrade by two decades and cost the count nothing. D1 sec. 5 measured 1.42 net decades of B2.2 -> B2.3 degradation that landed entirely on already-failing rows and was invisible to the gating total. These statistics are not censored and are reported beside it, always.

**4 OF THESE ROWS ARE SHARED WITH THE CUTOVER EXAM** — the same measurements, scored twice, so the two scorecards are NOT independent evidence on this axis:

| panel row | fold here | the exam point it IS |
|---|---:|---|
| `hofmann_ribose_pH3_MFT` | 0.024x | `mp_holdout_hofmann1998_ribose_cysteine_145C_20min_pH3 / MFT` |
| `hofmann_ribose_pH3_FFT` | 6.96x | `mp_holdout_hofmann1998_ribose_cysteine_145C_20min_pH3 / FFT` |
| `hofmann_ribose_pH7_MFT` | 0.00185x | `mp_holdout_hofmann1998_ribose_cysteine_145C_20min_pH7 / MFT` |
| `hofmann_ribose_pH7_FFT` | 0.00171x | `mp_holdout_hofmann1998_ribose_cysteine_145C_20min_pH7 / FFT` |

**GATING TRAJECTORY: B2.1 15 / 33 -> B2.2 12 / 33 -> B2.3 8 / 31 on the same denominator, 7 / 30 after Amendment 9's permanent demotion of `kang_140C_MFT`.**

The first three numbers are directly comparable: same rows, same bands, same scorer. The fourth is the honest total AFTER Amendment 9's permanent demotion of kang_140C_MFT. The last two are WAVE B8's two bases: Amendment 17 clause 2 retires the two `kang_switch_on_*` rows on evidence that their target is a hold-time artefact rather than a property of the chemistry, so the OLD BASIS (them counted, as they scored) is printed beside the NEW. A wave may not improve its score by removing rows, so the like-for-like is printed first and the removal is itemised.

- DEMOTED, not counted: `kang_140C_MFT` -- it would have PASSED. DEMOTED PERMANENTLY by Amendment 9 clause 1 (trigger disclosure: the charge defect this wave fixes was noticed through this row's scoring). Scored and reported, never counted.

### The retirement accounting (Amendment 17 clause 2) -- BOTH BASES

| basis | gating | what is in it |
|---|---|---|
| **OLD** (pre-Amendment-17) | **7 / 32** | the two `kang_switch_on_*` rows COUNTED, exactly as they scored |
| **NEW** (post-retirement) | **7 / 30** | those two removed; **0 rows added back** |

- RETIRED, not counted: `kang_switch_on_MFT` -- it would have FAILED at 0.23x. RETIRED, NOT FAILED (Amendment 17 clause 2). The MFT/FFT 'switch-on between 120 and 140 C' is a HOLD-TIME ARTEFACT of a single 120-minute slice through curves that peak and turn over. It fails on four axes, three of them inside its own source laboratory: (1) HOLD TIME -- in the same pot at the same three temperatures the 100->120 / 120->140 folds are 5.54x/2.61x at 40 min, 1.84x/3.25x at 80 min, 1.12x/4.26x at 120 min and 2.69x/1.62x at 180 min, i.e. the feature exists at ONE hold time; (2) CO-SUBSTRATE -- add an equimolar sugar and it vanishes (1.44x/1.30x); (3) AN INDEPENDENT LABORATORY -- Wang 2026's five-rung buffered ladder has both thiols peak MID-ladder and FALL; (4) the mechanism is visible in the raw time courses, where the 140 C curve has its maximum INSIDE the sampling window. A fixed-time slice across a family of curves whose maxima move left as temperature rises measures where each curve sits relative to its own peak, not an activation energy. Fitting a two-regime or threshold Ea form to it -- which kang2026_SI_extraction.md sec. 7a recommended -- would fit a sampling artefact into the physics.
- RETIRED, not counted: `kang_switch_on_FFT` -- it would have FAILED at 0.0452x. RETIRED, NOT FAILED (Amendment 17 clause 2) -- same evidence as kang_switch_on_MFT, and FFT's case is the stronger of the two: in the +xylose arm of its own source table the 120->140 fold is 0.96x, i.e. FFT FALLS across the leg it is supposed to switch on.

- ADDED, and it CANNOT GATE: `wang2026_MFT_peak_and_fall_125_over_115` -- scored FAIL. SEEN-BY-EXTRACTION and disclosed (Amendment 17 clause 3, B8). Printed in k6a sec. 4.3, which this wave was instructed to read. Pre-registered as N-1 with a predicted FAIL in a named direction before the scorer existed. Scored and reported, never counted.
- ADDED, and it CANNOT GATE: `wang2026_FFT_peak_and_fall_115_over_105` -- scored FAIL. SEEN-BY-EXTRACTION and disclosed (Amendment 17 clause 3, B8). Pre-registered as N-2. Scored and reported, never counted.
- ADDED, and it CANNOT GATE: `zhai_13C_exogenous_carbon_threshold` -- scored FAIL. SEEN-BY-EXTRACTION and disclosed (Amendment 17 clause 3, B8), and additionally OUT OF SCOPE in kind: the three compounds Zhai isotope-traced are thiophenes, which this module declares out of scope, and MFT/FFT were never traced. Pre-registered as N-3 with the scope gap stated in advance. Scored on an in-scope proxy, reported as a SCOPE GAP, never counted.
- ADDED, and it CANNOT GATE: `ames2001_excess_Ea_class_split` -- scored FAIL. SEEN-BY-EXTRACTION and disclosed (Amendment 17 clause 3, B8). Diagnostic for a second, independent reason: Ames measures a LOW-MOISTURE EXTRUDATE and k6a sec. 5.1 measures a ~2x aqueous-to-matrix gap on exactly these two barriers, with no transfer term in this model. Pre-registered as N-4. Never counted.

BOTH TOTALS ARE PRINTED SO THE RETIREMENT CANNOT READ AS SCORE LAUNDERING. If the retired rows were passing, removing them would LOWER the new-basis total and the retirement costs the wave something; if they were failing, removing them RAISES it and the reader is entitled to see by how much before deciding whether the evidence justifies it. The three rows Amendment 17 adds in their place are seen-by-extraction and CANNOT GATE, so the denominator genuinely falls by two and nothing is substituted in to hide it.

**Against B2.2: 12 / 33 gating.** B2.3 adds no row and removes none, so the denominators match and the totals ARE directly comparable. **1 FIXED, 5 REGRESSED.**

## B2.1 versus B2.2 versus B2.3, row by row

| row | gating | B2.1 fold | B2.1 | B2.2 fold | B2.2 | B2.3 fold | B2.3 | vs B2.2 |
|---|---|---:|---|---:|---|---:|---|---|
| `zhou_pH8_MFT` | yes | 0.628x | PASS | 0.343x | PASS | 0.727x | PASS | unchanged |
| `zhou_pH8_FFT` | yes | 0.00312x | **FAIL** | 0.381x | PASS | 0.257x | **FAIL** | REGRESSED |
| `zhou_pH8_MFTD` | yes | 0.701x | PASS | 4.33x | **FAIL** | 0.0663x | **FAIL** | unchanged |
| `zhou_pH8_ACTZ` | yes | 0.000719x | **FAIL** | 0.00028x | **FAIL** | 0.00197x | **FAIL** | unchanged |
| `zhou_pH8_FUR_arp_alone` | yes | 0.000744x | **FAIL** | 7.46x | **FAIL** | 0.133x | **FAIL** | unchanged |
| `zhou_MFT_shape_pH8_over_pH7` | yes | 1.28x | PASS | 0.509x | PASS | 0.871x | PASS | unchanged |
| `zhou_FFT_shape_pH8_over_pH7` | yes | 0.00524x | **FAIL** | 0.779x | PASS | 0.517x | PASS | unchanged |
| `zhou_pH6_MFT` | no | 0.0576x | **FAIL** | 1.82x | PASS | 2.25x | PASS | unchanged |
| `zhou_pH6_FFT` | no | 2.15x | PASS | 0.402x | PASS | 1.38x | PASS | unchanged |
| `zhou_pH6_MFTD` | no | 0.000259x | **FAIL** | 0.238x | **FAIL** | 0.0152x | **FAIL** | unchanged |
| `zhou_fig3_grid` | no | -- | -- | -- | -- | -- | -- | unchanged |
| `zhou_582_vs_665_consistency` | yes | 0.00032x | **FAIL** | 9.84e-07x | **FAIL** | 3.84e-06x | **FAIL** | unchanged |
| `whitfield_2001_pH65_collapse` | yes | 0.00408x | **FAIL** | 0.00437x | **FAIL** | 0.00667x | **FAIL** | unchanged |
| `cerny2003_NF_share_ceiling_95C` | yes | 0.736x | PASS | 14.1x | **FAIL** | 14.3x | **FAIL** | unchanged |
| `cerny2003_intact_skeleton_share_95C` | yes | 1.02x | PASS | 0.0147x | **FAIL** | 0.000272x | **FAIL** | unchanged |
| `route_mix_moves_with_temperature` | no | -- | PASS | -- | **FAIL** | -- | PASS | FIXED |
| `hofmann_dry_180C` | yes | -- | -- | -- | -- | -- | -- | unchanged |
| `hofmann_ribose_pH3_MFT` | yes | 0.0589x | **FAIL** | 0.0468x | **FAIL** | 0.024x | **FAIL** | unchanged |
| `hofmann_ribose_pH3_FFT` | yes | 24.8x | **FAIL** | 1.47x | PASS | 6.96x | **FAIL** | REGRESSED |
| `hofmann_ribose_pH7_MFT` | yes | 1.13x | PASS | 0.00539x | **FAIL** | 0.00185x | **FAIL** | unchanged |
| `hofmann_ribose_pH7_FFT` | yes | 0.379x | PASS | 0.208x | **FAIL** | 0.00171x | **FAIL** | unchanged |
| `meynier_MFT_pH_fold` | no | 0.0159x | **FAIL** | 3.87x | PASS | 11.6x | **FAIL** | REGRESSED |
| `meynier_FFT_pH_fold` | no | 5.31x | PASS | 12.2x | **FAIL** | 3.57e+03x | **FAIL** | unchanged |
| `meynier_FUR_pH_fold` | no | 0.21x | PASS | 0.416x | PASS | 0.671x | PASS | unchanged |
| `hofmann2002_brew_80C_FFT` | yes | 2.88x | PASS | 2.96x | PASS | 0.26x | **FAIL** | REGRESSED |
| `vanseeventer_50C_MFT_per_day` | yes | 1.38x | PASS | 1.66x | PASS | 0.00505x | **FAIL** | REGRESSED |
| `zhang_115C_MFT_consumed_share` | yes | 0.473x | PASS | 0.981x | PASS | 0.538x | PASS | unchanged |
| `zhou_120C_dimer_share_pH6` | yes | 0.00451x | **FAIL** | 0.131x | **FAIL** | 0.00678x | **FAIL** | unchanged |
| `zhou_120C_dimer_share_pH7` | yes | 0.747x | PASS | 0.233x | **FAIL** | 0.0307x | **FAIL** | unchanged |
| `zhou_120C_dimer_share_pH8` | yes | 1.12x | PASS | 12.6x | **FAIL** | 0.0912x | **FAIL** | unchanged |
| `zhou_dimer_share_pH_invariance` | yes | 187x | **FAIL** | 72.4x | **FAIL** | 10.2x | **FAIL** | unchanged |
| `cerny2007b_control_no_cysteine` | yes | 1.01x | PASS | 1.01x | PASS | 1.01x | PASS | unchanged |
| `cerny2007b_control_no_thiamine` | yes | 1.05x | PASS | 1.05x | PASS | 1.05x | PASS | unchanged |
| `cerny2007_1x_xylose_share` | yes | 2.74x | **FAIL** | 3.69x | **FAIL** | 3.8x | **FAIL** | unchanged |
| `cerny2007_2x_xylose_share` | yes | 1.01x | PASS | 1.19x | PASS | 1.14x | PASS | unchanged |
| `cerny2007_branch_responsiveness` | yes | 0.369x | **FAIL** | 0.323x | **FAIL** | 0.3x | **FAIL** | unchanged |
| `kang_140C_MFT` | no (DEMOTED) | 0.000656x | **FAIL** | 2.15x | PASS | 0.494x | PASS | unchanged |
| `kang_switch_on_MFT` | no | 0.000342x | **FAIL** | 2.45x | **FAIL** | 0.23x | **FAIL** | unchanged |
| `kang_140C_FFT` | yes | 1.52e-07x | **FAIL** | 0.0706x | **FAIL** | 0.0263x | **FAIL** | unchanged |
| `kang_switch_on_FFT` | no | 3.21e-07x | **FAIL** | 0.0792x | **FAIL** | 0.0452x | **FAIL** | unchanged |
| `kang_thiols_diverge_from_their_class` | no | -- | PASS | -- | PASS | -- | PASS | unchanged |
| `kang_140C_cys_conversion` | no | 1.38x | PASS | 1.24x | PASS | 1.09x | PASS | unchanged |
| `wang2026_MFT_peak_and_fall_125_over_115` | no (DEMOTED) | -- | -- | -- | -- | 9.92x | **FAIL** | NEW ROW |
| `wang2026_FFT_peak_and_fall_115_over_105` | no (DEMOTED) | -- | -- | -- | -- | 10.2x | **FAIL** | NEW ROW |
| `zhai_13C_exogenous_carbon_threshold` | no (DEMOTED) | -- | -- | -- | -- | -- | **FAIL** | NEW ROW |
| `ames2001_excess_Ea_class_split` | no (DEMOTED) | -- | -- | -- | -- | -- | **FAIL** | NEW ROW |
| `sun2019_pH9_over_pH3_free_FFT` | yes | 0.0735x | **FAIL** | 0.0569x | **FAIL** | 6.75x | **FAIL** | unchanged |
| `sun2019_temperature_ordering_inversion` | yes | -- | **FAIL** | -- | **FAIL** | -- | **FAIL** | unchanged |
| `amrani_hemaimi_onoff_switch` | no | -- | -- | -- | -- | -- | -- | unchanged |

A row marked REGRESSED is not automatically bad and a row marked FIXED is not automatically good: see the qualifications block. B2.3 adds no hold-out row and removes none, so NEW ROW should never appear -- if it does, the two scorers have drifted apart and the comparison is unsound. FIXED / REGRESSED are measured against B2.2; the B2.1 column is carried alongside so the whole three-wave trajectory is visible in one table.
...
- **`vanseeventer_50C_MFT_per_day` (FAIL)** -- THIS PASS SHOULD NOT BE COUNTED AS ONE. It was pre-registered as a failure and it passed the 3x band from the WRONG SIDE, by over-predicting: the model destroys ~99% of the MFT per day against a measured 59%, and it does so through the unassigned-decay and dimerisation channels, NOT through the C-5 oligomerisation channel that van Seeventer actually measured -- which still carries a rate of exactly zero. The FUNCTIONAL FORM is also still wrong: the measured loss is ZERO ORDER and every channel here is first or second order. The row tests the functional form, and on that test the model fails while landing inside the magnitude band by coincidence.

- **`cerny2007_branch_responsiveness` (FAIL)** -- A FAIL THAT CARRIES THE WAVE'S MAIN ARCHITECTURAL RESULT. This is the row a model with fixed branch fractions fails BY CONSTRUCTION, because such a model predicts exactly 1.00x. This model predicts 0.92x against a measured 3.07x. So the branch fraction DOES respond to concentration -- the architectural requirement is met and the no-fixed-fraction design is vindicated -- but it responds about half as strongly as measured. The residual is a magnitude error in the relative reaction ORDERS of the two routes, not the structural error the row was designed to catch. Reported as a fail, because it is one.

- **`zhou_pH8_FFT` (FAIL)** -- THE ROW B2.1 EXISTS TO FIX. B2 predicted 1.23e-07 against a measured 2.85e-03 -- 0.00x -- and its report named the cause: a structural pH slope of one decade per pH unit, right in shape and far too steep in level. B2.1 changed three things and NONE of them is a fitted pH parameter: the pKa are now evaluated at reaction temperature; no pH-sensitive product has a single fully-catalysed route any more; and the thiol's pH-dependent CONSUMPTION lane, which Kumazawa measures directly at 121 C and which B2 did not have at all, now carries the part of the pH response that belongs to it. B2.1 predicts 0.000733 against the same measured 2.849e-03, a fold of 0.257x. Whether that is a pass or a fail, it is the single number by which this wave should be judged.

- **`hofmann2002_brew_80C_FFT` (FAIL)** -- THE FAILURE THE DECLARATION PREDICTED AND ASKED FOR, and B2.1 attacks it from two measured directions rather than by tuning. FIRST: B2 pinned every fitted decay lump to its 145 C value and evaluated it unchanged at 80 C, so k_fft_decay alone predicted several times the measured brew loss; those lumps now carry the lumped Ea, which is a WEAKER claim than temperature-independence, not a stronger one. SECOND: Stack 2018's measured van 't Hoff enthalpy is NEGATIVE, so the thiol-electrophile equilibrium constant FALLS 5x between 30 and 80 C -- heating releases bound thiol. B2 could not express that, because its K was a temperature-independent number that the source does not print. B2 lost FFT 18x too fast; B2.1 is at 0.26x.

- **`kang_140C_MFT` (PASS)** -- THE NEW EXAM, AND THE ONE WHOSE DIRECTION OF FAILURE MATTERS MOST. Amendment 5 pre-registers that a single-Ea branch UNDER-predicts here by 3.8x, because the measured behaviour is non-Arrhenius: free-thiol formation switches on between 120 and 140 C. This module is not single-Ea at that node -- its thiol-forming flux is [carbon skeleton] x [sulfide] and the sulfide is gated behind Zheng & Ho's MEASURED 133 kJ/mol, the largest barrier in the module by a factor of two, against a lumped formation barrier for everything else. So it can also OVERSHOOT, and an overshoot is a failure of the same seriousness as an undershoot: it would mean the switch-on mechanism is real but mis-sized. MEASURED 5.907 ug/L; PREDICTED 2.555e-05 mmol/L, fold 0.494x.

## Row by row -- no averaging, no dropping

| row | group | gating | observed | predicted | fold | pass |
|---|---|---|---:|---:|---:|---|
| `zhou_pH8_MFT` | Zhou pH-8 column | yes | 0.004604 | 0.003349 | 0.73x | PASS |
| `zhou_pH8_FFT` | Zhou pH-8 column | yes | 0.002849 | 0.0007326 | 0.26x | **FAIL** |
| `zhou_pH8_MFTD` | Zhou pH-8 column | yes | 0.0002212 | 1.467e-05 | 0.07x | **FAIL** |
| `zhou_pH8_ACTZ` | Zhou pH-8 column | yes | 0.00458 | 9.017e-06 | 0.00x | **FAIL** |
| `zhou_pH8_FUR_arp_alone` | Zhou pH-8 column | yes | 0.004544 | 0.0006036 | 0.13x | **FAIL** |
| `zhou_MFT_shape_pH8_over_pH7` | Zhou pH-8 column | yes | 0.3309 | 0.2883 | 0.87x | PASS |
| `zhou_FFT_shape_pH8_over_pH7` | Zhou pH-8 column | yes | 0.4291 | 0.2217 | 0.52x | PASS |
| `zhou_pH6_MFT` | Zhou pH-6 column (DIAGNOSTIC) | no | 0.006105 | 0.01372 | 2.25x | PASS |
| `zhou_pH6_FFT` | Zhou pH-6 column (DIAGNOSTIC) | no | 0.007127 | 0.009847 | 1.38x | PASS |
| `zhou_pH6_MFTD` | Zhou pH-6 column (DIAGNOSTIC) | no | 0.0002638 | 4e-06 | 0.02x | **FAIL** |
| `zhou_fig3_grid` | Zhou Fig. 3 grid | no | -- | -- | -- | _not scored_ |
| `zhou_582_vs_665_consistency` | Zhou cross-system pair | yes | 0.8757 | 3.365e-06 | 0.00x | **FAIL** |
| `whitfield_2001_pH65_collapse` | Whitfield 2001 pH collapse | yes | 150 | 1.001 | 0.01x | **FAIL** |
| `cerny2003_NF_share_ceiling_95C` | Cerny 2003, 95 C | yes | 0.07 | 0.9997 | 14.28x | **FAIL** |
| `cerny2003_intact_skeleton_share_95C` | Cerny 2003, 95 C | yes | 0.93 | 0.0002527 | 0.00x | **FAIL** |
| `route_mix_moves_with_temperature` | Cerny 2003, 95 C | no | -- | -- | -- | PASS |
| `hofmann_dry_180C` | Hofmann dry-180 | yes | -- | -- | -- | _not scored_ |
| `hofmann_ribose_pH3_MFT` | Hofmann pH axis | yes | 0.004844 | 0.0001163 | 0.02x | **FAIL** |
| `hofmann_ribose_pH3_FFT` | Hofmann pH axis | yes | 0.002006 | 0.01397 | 6.96x | **FAIL** |
| `hofmann_ribose_pH7_MFT` | Hofmann pH axis | yes | 0.000219 | 4.052e-07 | 0.00x | **FAIL** |
| `hofmann_ribose_pH7_FFT` | Hofmann pH axis | yes | 0.0001051 | 1.796e-07 | 0.00x | **FAIL** |
| `meynier_MFT_pH_fold` | Meynier directional | no | 152 | 1767 | 11.62x | **FAIL** |
| `meynier_FFT_pH_fold` | Meynier directional | no | 6.1 | 2.178e+04 | 3571.07x | **FAIL** |
| `meynier_FUR_pH_fold` | Meynier directional | no | 25 | 16.79 | 0.67x | PASS |
| `hofmann2002_brew_80C_FFT` | thiol sinks | yes | 0.023 | 0.005974 | 0.26x | **FAIL** |
| `vanseeventer_50C_MFT_per_day` | thiol sinks | yes | 0.59 | 0.002979 | 0.01x | **FAIL** |
| `zhang_115C_MFT_consumed_share` | thiol sinks | yes | 0.11 | 0.0592 | 0.54x | PASS |
| `zhou_120C_dimer_share_pH6` | thiol sinks | yes | 0.086 | 0.000583 | 0.01x | **FAIL** |
| `zhou_120C_dimer_share_pH7` | thiol sinks | yes | 0.065 | 0.001998 | 0.03x | **FAIL** |
| `zhou_120C_dimer_share_pH8` | thiol sinks | yes | 0.096 | 0.00876 | 0.09x | **FAIL** |
| `zhou_dimer_share_pH_invariance` | thiol sinks | yes | 1.48 | 15.03 | 10.15x | **FAIL** |
...
| `cerny2007_1x_xylose_share` | Cerny concentration pair | yes | 0.15 | 0.5703 | 3.80x | **FAIL** |
| `cerny2007_2x_xylose_share` | Cerny concentration pair | yes | 0.46 | 0.5252 | 1.14x | PASS |
| `cerny2007_branch_responsiveness` | Cerny concentration pair | yes | 3.067 | 0.9209 | 0.30x | **FAIL** |
| `kang_140C_MFT` | Kang 140 C (NEW gating hold-out) | no | 5.174e-05 | 2.555e-05 | 0.49x | PASS |
| `kang_switch_on_MFT` | Kang 140 C (NEW gating hold-out) | no | 4.256 | 0.9773 | 0.23x | **FAIL** |
| `kang_140C_FFT` | Kang 140 C (NEW gating hold-out) | yes | 0.0001002 | 2.639e-06 | 0.03x | **FAIL** |
| `kang_switch_on_FFT` | Kang 140 C (NEW gating hold-out) | no | 2.785 | 0.1258 | 0.05x | **FAIL** |
| `kang_thiols_diverge_from_their_class` | Kang 140 C (NEW gating hold-out) | no | -- | -- | -- | PASS |
| `kang_140C_cys_conversion` | Kang 140 C (NEW gating hold-out) | no | 0.626 | 0.6813 | 1.09x | PASS |
| `wang2026_MFT_peak_and_fall_125_over_115` | Wang 2026 five-rung ladder (B8 replacement) | no | 0.3 | 2.977 | 9.92x | **FAIL** |
| `wang2026_FFT_peak_and_fall_115_over_105` | Wang 2026 five-rung ladder (B8 replacement) | no | 0.15 | 1.526 | 10.17x | **FAIL** |
| `zhai_13C_exogenous_carbon_threshold` | Zhai 13C5 isotope threshold (B8 replacement) | no | -- | -- | -- | **FAIL** |
| `ames2001_excess_Ea_class_split` | Ames 2001 extrusion excess-Ea (B8 replacement) | no | -- | -- | -- | **FAIL** |
| `sun2019_pH9_over_pH3_free_FFT` | Sun 2019 pH-9 column | yes | 0.1481 | 1 | 6.75x | **FAIL** |
...
- source: Wang 2026 Fig. 5 (wang2026_extraction.md sec. 4): MFT fold 0.3 over 115 -> 125 C, purified Amadori pot, 0.2 M PBS, five rungs 85-125 C
...
- source: Wang 2026 Fig. 5 (wang2026_extraction.md sec. 4): FFT fold 0.15 over 105 -> 115 C, purified Amadori pot, 0.2 M PBS, five rungs 85-125 C
...
- source: Zhai Food Chem 2023 Table 2 (zhai2023foodchem_extraction.md sec. 6): exogenous-carbon incorporation goes 0 % -> 19-21 % -> 34-45 % at 100 / 120 / 140 C for 3-thiophenethiol, thieno[3,2-b]thiophene and 2-methylthieno[2,3-b]thiophene. The threshold is at 100->120 C, NOT at 120->140 C where the retired switch-on put it.
...
- source: Ames 2001 (ames2001_extraction.md; k6a sec. 4.5): on the 120->150 C leg of the xylose / pH 7.5 extrusion ladder the EXCESS activation energy over the bulk is POSITIVE for the free thiols (FFT +46.7, MFT +33.5, aliphatic S +108.5) and NEGATIVE for the ring-closure products (thiazoles -42.7, thiophenes -6.5). A SIGN SPLIT, scored as one.
