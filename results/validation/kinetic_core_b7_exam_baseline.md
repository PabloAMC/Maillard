...
- **GEOMETRIC-MEAN FOLD, on the same points:** core over all scored **60.96x**; on the paired subset core **60.96x** vs old **16.8x**. A median moves only when a row crosses the middle of the pool; a geometric mean moves whenever any row moves, including a row that was already failing.
