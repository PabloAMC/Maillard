# Experiment Request `requested_cys_ribose_140c_hofmann1998_2_methyl_3_furanthiol_rank5`

VoI rank **#5**, score **3.77**.
...
- ODT: 0.007 µg/kg; decision relevance 4.69
- Rationale: CI width 2.68 dex; ≈5e+04× ODT (decision_relevance=4.69); critical meaty odorant in a FREE-PRECURSOR system — the open question is absolute yield vs precursor dose and temperature, not matrix transfer
...
- [ ] Confirm target compound identity and calibrate over a range spanning BOTH the published value (≈ 342 ppb) and the model's 90% CI midpoint (≈ 166.623 ppb). They differ by 2.05× — that disagreement is *why* this experiment is ranked, so a calibration range covering only one of them cannot resolve it.
...
- [ ] Record the measured `conc_ppb` and `uncertainty_pct` (1σ relative) under `measured_volatiles.2-methyl-3-furanthiol` in the pre-filled intake YAML.
...
- Pre-filled intake YAML: `data/protocols/requested_cys_ribose_140c_hofmann1998_2_methyl_3_furanthiol_rank5.yaml`
