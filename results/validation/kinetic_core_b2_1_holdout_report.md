# Kinetic core, Build Wave B2.1 -- the pre-registered hold-out scorecard

Predicted from the FROZEN fit in `results/validation/kinetic_core_b2_1_fit_report.json`. **No parameter was changed after the fit**; there is no optimiser in the scoring script.

**GATING: 15 / 33 passed.** Diagnostic: 6 / 9. Unscoreable or explicitly deferred: 3.

**Against B2: 7 / 27 gating.** B2.1 adds 8 rows the declaration's Amendments 4 and 5 created, so the denominators differ and the two totals are NOT directly comparable; the row-by-row table below is.

## B2 versus B2.1, row by row

| row | gating | B2 fold | B2 | B2.1 fold | B2.1 | |
|---|---|---:|---|---:|---|---|
| `zhou_pH8_MFT` | yes | 0.236x | **FAIL** | 0.628x | PASS | FIXED |
| `zhou_pH8_FFT` | yes | 4.32e-05x | **FAIL** | 0.00312x | **FAIL** | unchanged |
| `zhou_pH8_MFTD` | yes | 0.0187x | **FAIL** | 0.701x | PASS | FIXED |
| `zhou_pH8_ACTZ` | yes | 0.00049x | **FAIL** | 0.000719x | **FAIL** | unchanged |
| `zhou_pH8_FUR_arp_alone` | yes | 0.00411x | **FAIL** | 0.000744x | **FAIL** | unchanged |
| `zhou_MFT_shape_pH8_over_pH7` | yes | 1.81x | PASS | 1.28x | PASS | unchanged |
| `zhou_FFT_shape_pH8_over_pH7` | yes | 0.000374x | **FAIL** | 0.00524x | **FAIL** | unchanged |
| `zhou_pH6_MFT` | no | 0.0142x | **FAIL** | 0.0576x | **FAIL** | unchanged |
| `zhou_pH6_FFT` | no | 0.139x | **FAIL** | 2.15x | PASS | FIXED |
| `zhou_pH6_MFTD` | no | 8.6e-05x | **FAIL** | 0.000259x | **FAIL** | unchanged |
| `zhou_fig3_grid` | no | -- | -- | -- | -- | unchanged |
| `zhou_582_vs_665_consistency` | yes | 4.28e-05x | **FAIL** | 0.00032x | **FAIL** | unchanged |
| `whitfield_2001_pH65_collapse` | yes | 0.0019x | **FAIL** | 0.00408x | **FAIL** | unchanged |
| `cerny2003_NF_share_ceiling_95C` | yes | 5.47x | **FAIL** | 0.736x | PASS | FIXED |
| `cerny2003_intact_skeleton_share_95C` | yes | 0.663x | PASS | 1.02x | PASS | unchanged |
| `route_mix_moves_with_temperature` | no | -- | **FAIL** | -- | PASS | FIXED |
| `hofmann_dry_180C` | yes | -- | -- | -- | -- | unchanged |
| `hofmann_ribose_pH3_MFT` | yes | 0.0808x | **FAIL** | 0.0589x | **FAIL** | unchanged |
| `hofmann_ribose_pH3_FFT` | yes | 16.3x | **FAIL** | 24.8x | **FAIL** | unchanged |
| `hofmann_ribose_pH7_MFT` | yes | 60.2x | **FAIL** | 1.13x | PASS | FIXED |
| `hofmann_ribose_pH7_FFT` | yes | 0.0701x | **FAIL** | 0.379x | PASS | FIXED |
| `meynier_MFT_pH_fold` | no | 0.00057x | **FAIL** | 0.0159x | **FAIL** | unchanged |
| `meynier_FFT_pH_fold` | no | 13.2x | **FAIL** | 5.31x | PASS | FIXED |
| `meynier_FUR_pH_fold` | no | 5.02x | PASS | 0.21x | PASS | unchanged |
| `hofmann2002_brew_80C_FFT` | yes | 18x | **FAIL** | 2.88x | PASS | FIXED |
| `vanseeventer_50C_MFT_per_day` | yes | 1.69x | PASS | 1.38x | PASS | unchanged |
| `zhang_115C_MFT_consumed_share` | yes | 1.1x | PASS | 0.473x | PASS | unchanged |
| `zhou_120C_dimer_share_pH6` | yes | 0.00608x | **FAIL** | 0.00451x | **FAIL** | unchanged |
| `zhou_120C_dimer_share_pH7` | yes | 0.212x | **FAIL** | 0.747x | PASS | FIXED |
| `zhou_120C_dimer_share_pH8` | yes | 0.0789x | **FAIL** | 1.12x | PASS | FIXED |
| `zhou_dimer_share_pH_invariance` | yes | 17.8x | **FAIL** | 187x | **FAIL** | unchanged |
...
| `cerny2007_1x_xylose_share` | yes | 3.04x | **FAIL** | 2.74x | **FAIL** | unchanged |
| `cerny2007_2x_xylose_share` | yes | 1.47x | PASS | 1.01x | PASS | unchanged |
| `cerny2007_branch_responsiveness` | yes | 0.485x | **FAIL** | 0.369x | **FAIL** | unchanged |
| `kang_140C_MFT` | yes | -- | -- | 0.000656x | **FAIL** | NEW ROW |
| `kang_switch_on_MFT` | yes | -- | -- | 0.000342x | **FAIL** | NEW ROW |
| `kang_140C_FFT` | yes | -- | -- | 1.52e-07x | **FAIL** | NEW ROW |
| `kang_switch_on_FFT` | yes | -- | -- | 3.21e-07x | **FAIL** | NEW ROW |
| `kang_thiols_diverge_from_their_class` | no | -- | -- | -- | PASS | NEW ROW |
| `kang_140C_cys_conversion` | no | -- | -- | 1.38x | PASS | NEW ROW |
| `sun2019_pH9_over_pH3_free_FFT` | yes | -- | -- | 0.0735x | **FAIL** | NEW ROW |
| `sun2019_temperature_ordering_inversion` | yes | -- | -- | -- | **FAIL** | NEW ROW |
| `amrani_hemaimi_onoff_switch` | no | -- | -- | -- | -- | unchanged |

A row marked REGRESSED is not automatically bad and a row marked FIXED is not automatically good: see the qualifications block, which names one B2 pass that its own report refused to count and which B2.1 expected to lose. Rows marked NEW ROW did not exist in B2 and were added by Amendments 4 and 5.
...
- **`cerny2007_branch_responsiveness` (FAIL)** -- A FAIL THAT CARRIES THE WAVE'S MAIN ARCHITECTURAL RESULT. This is the row a model with fixed branch fractions fails BY CONSTRUCTION, because such a model predicts exactly 1.00x. This model predicts 1.13x against a measured 3.07x. So the branch fraction DOES respond to concentration -- the architectural requirement is met and the no-fixed-fraction design is vindicated -- but it responds about half as strongly as measured. The residual is a magnitude error in the relative reaction ORDERS of the two routes, not the structural error the row was designed to catch. Reported as a fail, because it is one.

- **`zhou_pH8_FFT` (FAIL)** -- THE ROW B2.1 EXISTS TO FIX. B2 predicted 1.23e-07 against a measured 2.85e-03 -- 0.00x -- and its report named the cause: a structural pH slope of one decade per pH unit, right in shape and far too steep in level. B2.1 changed three things and NONE of them is a fitted pH parameter: the pKa are now evaluated at reaction temperature; no pH-sensitive product has a single fully-catalysed route any more; and the thiol's pH-dependent CONSUMPTION lane, which Kumazawa measures directly at 121 C and which B2 did not have at all, now carries the part of the pH response that belongs to it. B2.1 predicts 8.88e-06 against the same measured 2.849e-03, a fold of 0.00312x. Whether that is a pass or a fail, it is the single number by which this wave should be judged.

- **`hofmann2002_brew_80C_FFT` (PASS)** -- THE FAILURE THE DECLARATION PREDICTED AND ASKED FOR, and B2.1 attacks it from two measured directions rather than by tuning. FIRST: B2 pinned every fitted decay lump to its 145 C value and evaluated it unchanged at 80 C, so k_fft_decay alone predicted several times the measured brew loss; those lumps now carry the lumped Ea, which is a WEAKER claim than temperature-independence, not a stronger one. SECOND: Stack 2018's measured van 't Hoff enthalpy is NEGATIVE, so the thiol-electrophile equilibrium constant FALLS 5x between 30 and 80 C -- heating releases bound thiol. B2 could not express that, because its K was a temperature-independent number that the source does not print. B2 lost FFT 18x too fast; B2.1 is at 2.9x.

- **`kang_140C_MFT` (FAIL)** -- THE NEW EXAM, AND THE ONE WHOSE DIRECTION OF FAILURE MATTERS MOST. Amendment 5 pre-registers that a single-Ea branch UNDER-predicts here by 3.8x, because the measured behaviour is non-Arrhenius: free-thiol formation switches on between 120 and 140 C. This module is not single-Ea at that node -- its thiol-forming flux is [carbon skeleton] x [sulfide] and the sulfide is gated behind Zheng & Ho's MEASURED 133 kJ/mol, the largest barrier in the module by a factor of two, against a lumped formation barrier for everything else. So it can also OVERSHOOT, and an overshoot is a failure of the same seriousness as an undershoot: it would mean the switch-on mechanism is real but mis-sized. MEASURED 5.907 ug/L; PREDICTED 3.397e-08 mmol/L, fold 0.000656x.

## Row by row -- no averaging, no dropping

| row | group | gating | observed | predicted | fold | pass |
|---|---|---|---:|---:|---:|---|
| `zhou_pH8_MFT` | Zhou pH-8 column | yes | 0.004604 | 0.002893 | 0.63x | PASS |
| `zhou_pH8_FFT` | Zhou pH-8 column | yes | 0.002849 | 8.879e-06 | 0.00x | **FAIL** |
| `zhou_pH8_MFTD` | Zhou pH-8 column | yes | 0.0002212 | 0.0001551 | 0.70x | PASS |
| `zhou_pH8_ACTZ` | Zhou pH-8 column | yes | 0.00458 | 3.291e-06 | 0.00x | **FAIL** |
| `zhou_pH8_FUR_arp_alone` | Zhou pH-8 column | yes | 0.004544 | 3.381e-06 | 0.00x | **FAIL** |
| `zhou_MFT_shape_pH8_over_pH7` | Zhou pH-8 column | yes | 0.3309 | 0.4219 | 1.28x | PASS |
| `zhou_FFT_shape_pH8_over_pH7` | Zhou pH-8 column | yes | 0.4291 | 0.002246 | 0.01x | **FAIL** |
| `zhou_pH6_MFT` | Zhou pH-6 column (DIAGNOSTIC) | no | 0.006105 | 0.0003519 | 0.06x | **FAIL** |
| `zhou_pH6_FFT` | Zhou pH-6 column (DIAGNOSTIC) | no | 0.007127 | 0.01532 | 2.15x | PASS |
| `zhou_pH6_MFTD` | Zhou pH-6 column (DIAGNOSTIC) | no | 0.0002638 | 6.827e-08 | 0.00x | **FAIL** |
| `zhou_fig3_grid` | Zhou Fig. 3 grid | no | -- | -- | -- | _not scored_ |
| `zhou_582_vs_665_consistency` | Zhou cross-system pair | yes | 0.8757 | 0.0002805 | 0.00x | **FAIL** |
| `whitfield_2001_pH65_collapse` | Whitfield 2001 pH collapse | yes | 150 | 0.612 | 0.00x | **FAIL** |
| `cerny2003_NF_share_ceiling_95C` | Cerny 2003, 95 C | yes | 0.07 | 0.05153 | 0.74x | PASS |
| `cerny2003_intact_skeleton_share_95C` | Cerny 2003, 95 C | yes | 0.93 | 0.9442 | 1.02x | PASS |
| `route_mix_moves_with_temperature` | Cerny 2003, 95 C | no | -- | -- | -- | PASS |
| `hofmann_dry_180C` | Hofmann dry-180 | yes | -- | -- | -- | _not scored_ |
| `hofmann_ribose_pH3_MFT` | Hofmann pH axis | yes | 0.004844 | 0.0002855 | 0.06x | **FAIL** |
| `hofmann_ribose_pH3_FFT` | Hofmann pH axis | yes | 0.002006 | 0.04971 | 24.78x | **FAIL** |
| `hofmann_ribose_pH7_MFT` | Hofmann pH axis | yes | 0.000219 | 0.000247 | 1.13x | PASS |
| `hofmann_ribose_pH7_FFT` | Hofmann pH axis | yes | 0.0001051 | 3.983e-05 | 0.38x | PASS |
| `meynier_MFT_pH_fold` | Meynier directional | no | 152 | 2.423 | 0.02x | **FAIL** |
| `meynier_FFT_pH_fold` | Meynier directional | no | 6.1 | 32.38 | 5.31x | PASS |
| `meynier_FUR_pH_fold` | Meynier directional | no | 25 | 5.244 | 0.21x | PASS |
| `hofmann2002_brew_80C_FFT` | thiol sinks | yes | 0.023 | 0.06631 | 2.88x | PASS |
| `vanseeventer_50C_MFT_per_day` | thiol sinks | yes | 0.59 | 0.8162 | 1.38x | PASS |
| `zhang_115C_MFT_consumed_share` | thiol sinks | yes | 0.11 | 0.052 | 0.47x | PASS |
| `zhou_120C_dimer_share_pH6` | thiol sinks | yes | 0.086 | 0.000388 | 0.00x | **FAIL** |
| `zhou_120C_dimer_share_pH7` | thiol sinks | yes | 0.065 | 0.04857 | 0.75x | PASS |
| `zhou_120C_dimer_share_pH8` | thiol sinks | yes | 0.096 | 0.1072 | 1.12x | PASS |
| `zhou_dimer_share_pH_invariance` | thiol sinks | yes | 1.48 | 276.3 | 186.66x | **FAIL** |
...
| `cerny2007_1x_xylose_share` | Cerny concentration pair | yes | 0.15 | 0.4117 | 2.74x | **FAIL** |
| `cerny2007_2x_xylose_share` | Cerny concentration pair | yes | 0.46 | 0.4663 | 1.01x | PASS |
| `cerny2007_branch_responsiveness` | Cerny concentration pair | yes | 3.067 | 1.133 | 0.37x | **FAIL** |
| `kang_140C_MFT` | Kang 140 C (NEW gating hold-out) | yes | 5.174e-05 | 3.397e-08 | 0.00x | **FAIL** |
| `kang_switch_on_MFT` | Kang 140 C (NEW gating hold-out) | yes | 4.256 | 0.001458 | 0.00x | **FAIL** |
| `kang_140C_FFT` | Kang 140 C (NEW gating hold-out) | yes | 0.0001002 | 1.523e-11 | 0.00x | **FAIL** |
| `kang_switch_on_FFT` | Kang 140 C (NEW gating hold-out) | yes | 2.785 | 8.952e-07 | 0.00x | **FAIL** |
| `kang_thiols_diverge_from_their_class` | Kang 140 C (NEW gating hold-out) | no | -- | -- | -- | PASS |
| `kang_140C_cys_conversion` | Kang 140 C (NEW gating hold-out) | no | 0.626 | 0.8647 | 1.38x | PASS |
| `sun2019_pH9_over_pH3_free_FFT` | Sun 2019 pH-9 column | yes | 0.1481 | 0.01089 | 0.07x | **FAIL** |
