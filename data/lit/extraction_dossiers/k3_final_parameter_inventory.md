...
| B2.7 | furfural falls **48.6× to more than 1510×** over pH 4.5→6.5 in **aqueous** model systems, but is **unaffected pH 4→9** in roasted seed at **a_w ≈ 0.3** | methionine 92.4 → 1.9 (48.6×, the only uncensored series); glycine 151.0 → tr (**> 1510×**); lysine 10.7 → nd (**> 1070×**); cysteine 3.9 → tr (**> 39×**), µg per 10 mg ribose | two matrices | Meynier 1995 Tables 1–4 vs Laemont (PH-07) |
...
| B2.10 | MFT falls **> 152×**, FFT **6.1×**, furfural **48.6× to more than 1510×** over pH 4.5 → 6.5 | constant-pH, 4-amino-acid, single-sugar factorial, ~59 series | Meynier 1995 |

**CORRECTION 2026-09-09 (B2.7 and B2.10, furfural only).** Both rows read "15–49×" until this date. The MFT and FFT figures in the same rows reproduce exactly from Meynier 1995's Table 3 (15.2 → tr and 8.5/1.4 = 6.07), and the furfural figure does not: **49 is methionine's 48.6×, and 15 is reproducible as glycine's 151.0/10.1, which is that series' pH 4.5 → 5.0 STEP and not its 4.5 → 6.5 span.** The PDF's text layer interleaves Table 1's labels so that furfural's 151.0 appears against the 2-furylmethanol label and the pH 6.5 column is dropped, which is the likely origin. Three of the four series end censored, so their folds are lower bounds. The direction is unchanged and the conclusion the rows support -- that no family-level pH term can pass -- is strengthened, not weakened: the true spread across four amino acids is wider than the range these rows claimed.
